...
# Про адрес ИЗДАТЕЛЯ здесь проверок нет намеренно: на стенде он живёт в docker-сети клиента, то
# есть приватный и в одной подсети с ним. Завернуть его в туннель мало (F2 режет приватные dst
# раньше G1), а не заворачивать — значит мерить бридж, а не туннель. Живую форму «издатель в
# DENY» стенд всё равно воспроизводит, и куда точнее: его адрес лежит в Citadel_DENY_DSTS (см.
# entrypoint-exit.sh), поэтому ТЕСТ 20 гоняет admin-плоскость ровно тем путём, на котором она
# ломалась, — VIP → DNAT → dst издателя под запретом G1. Красный ТЕСТ 20 = вернулась регрессия.
