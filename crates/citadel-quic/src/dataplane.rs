...
pub async fn pump(
    ...
    let sender = tokio::spawn(async move {
        while let Some(pkt) = tun_to_net_rx.recv().await {
            ...
            // Пакет из TUN, адресованный самому exit'у, — петля собственного транспорта (наш
            // сокет к exit'у не исключён из туннеля). Проверка стоит ОТДЕЛЬНО от «чужого src», а
            // не внутри неё, как было: когда маршрут к exit уходит в TUN, ядро выбирает адресом
            // источника адрес самого TUN — то есть НАШ назначенный, и петля с проверкой на чужой
            // src не считалась вовсе. Именно этот счётчик отличает «сеть плохая» от «мы сами
            // завернули свой транспорт в свой туннель».
