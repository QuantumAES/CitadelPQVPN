...
/// Сколько держится вердикт «QUIC/UDP в этой сети не работает» (см. [`udp_one_way_recent`]).
...
/// Когда в последний раз QUIC/UDP оказался односторонним. Глобально на процесс, а не поле
/// контроллера, СПЕЦИАЛЬНО: приложение создаёт НОВЫЙ `VpnController` на каждое нажатие
/// «Подключить» (см. `app/rust/src/api/citadel.rs`), и вердикт, живущий в структуре, умирал бы
/// вместе с ней. Из-за этого каждое подключение на «плохой» сети начиналось одинаково: сессия по
/// QUIC/UDP, 4с мёртвого туннеля, разрыв, переустановка поверх obfs-TCP — и второй токен из
/// кошелька за то же самое подключение. Помним вердикт — и следующая сессия сразу идёт рабочим
/// транспортом.
static UDP_ONE_WAY: Mutex<Option<std::time::Instant>> = Mutex::new(None);

/// Отметить: QUIC/UDP в текущей сети не несёт наш трафик.
fn note_udp_one_way() {
    *UDP_ONE_WAY.lock().unwrap() = Some(std::time::Instant::now());
}

/// Забыть вердикт (сменилась сеть / obfs-TCP оказался не лучше — судить QUIC не за что).
fn clear_udp_one_way() {
    *UDP_ONE_WAY.lock().unwrap() = None;
}

/// Свеж ли вердикт «QUIC/UDP односторонний» (в пределах [`UDP_VERDICT_TTL`]).
fn udp_one_way_recent() -> bool {
    matches!(*UDP_ONE_WAY.lock().unwrap(), Some(at) if at.elapsed() < UDP_VERDICT_TTL)
}
...
impl VpnController {
    ...
    pub async fn connect(&self, mut cfg: ClientConfig, provider: Arc<dyn TunProvider>) -> Result<()> {
        ...
        // Идти ли сразу obfs-TCP. Ставится, когда сессия по QUIC/UDP оказалась ОДНОСТОРОННЕЙ
        // (наши датаграммы перестали доезжать: узкий MTU пути/потери — см. `dataplane::PumpExit`).
        // Реконнект тем же транспортом в такой сети упирается ровно в то же самое, и абонент
        // получает бесконечное «переподключение» вместо связи. Начальное значение — из вердикта
        // ПРОШЛОГО подключения (см. [`udp_one_way_recent`]): сеть за минуту не меняется, а платит
        // за повторную пробу человек — четырьмя секундами мёртвого туннеля и лишним токеном.
        let mut prefer_tcp = udp_one_way_recent() && cfg.transport_psk().is_some();
        if prefer_tcp {
            eprintln!(
                "[vpn] в этой сети QUIC/UDP уже оказывался односторонним — начинаю сразу с \
                 obfs-TCP (вердикт сбрасывается сменой сети и через {} мин)",
...
mod tests {
    ...
    fn udp_verdict_outlives_controller_but_expires_and_clears() {
        clear_udp_one_way();
        assert!(!udp_one_way_recent(), "чистый процесс — QUIC не осуждён");
        note_udp_one_way();
        assert!(udp_one_way_recent(), "вердикт вынесен — следующая сессия идёт obfs-TCP");
        // Смена сети / не поднявшийся obfs-TCP снимают вердикт: судить QUIC больше не за что.
        clear_udp_one_way();
        assert!(!udp_one_way_recent(), "вердикт снят — обычный порядок транспортов");
        // Протухание: вердикт старше TTL не считается (сеть могла и починиться).
        *UDP_ONE_WAY.lock().unwrap() =
            std::time::Instant::now().checked_sub(UDP_VERDICT_TTL + Duration::from_secs(1));
        assert!(!udp_one_way_recent(), "вердикт протух — пробуем QUIC заново");
        clear_udp_one_way();
    }
