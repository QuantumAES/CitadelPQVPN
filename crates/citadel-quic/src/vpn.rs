        ...
        loop {
            ...
            if let (Some(f), false) = (&refresher, have_unspent_token) {
                ...
                match fetched {
                    ...
                    // прямо перед этой — там она названа точно (нет сети / выбрана квота эпохи с
                    // остатком времени / отказ реестра), а гадать здесь ещё раз значит спорить с
                    // ним же и уводить диагноз в сторону. Идти к exit'у с потраченным токеном
                    // бессмысленно — он ответит «auth-failed», и в UI встанет ложная причина.
                    None if attempted => {
                        let why = "нет свежего Layer-1 токена — причина в строке [token] выше; \
                                   жду и пробую снова";
