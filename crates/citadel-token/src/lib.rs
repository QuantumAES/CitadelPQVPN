...
fn connect_authenticated_issuer(
    ...
    let tcp = tcp.ok_or_else(|| IssuerUnreachable { addr: issuer_addr.to_string() })?;
...
pub fn fetch_tokens(
    ...
    for i in 0..count {
        ...
        let resp = match read_frame(&mut conn) {
            ...
            // Ни одного токена, и издатель закрыл соединение сам (чистый EOF: он дочитал наш
            // ослеплённый элемент и вышел из цикла). Это НЕ гадание — до сюда мы дошли только
            // потому, что гейт пройден и кадр эпохи получен, а других выходов из цикла выдачи у
            // издателя нет: значит, выбрана квота эпохи (`take_quota`). Раньше здесь стоял список
            // из трёх гипотез («квота? отзыв? аренда?»), человек читал его как «что-то сломалось»
            // и шёл чинить сеть или firewall — теперь причина названа одна и со сроком, а само
            // `e` («failed to fill whole buffer») остаётся источником в цепочке.
            Err(e) if e.kind() == io::ErrorKind::UnexpectedEof => {
                return Err(anyhow::Error::new(e).context(QuotaExhausted {
                    epoch: epoch.epoch,
                    epoch_secs: epoch.epoch_secs,
                }))
            }
            // Сброс/таймаут — это оборванная связь, а не политика издателя. Разделять обязательно:
            // по квоте клиент замолкает до конца эпохи ([`QuotaExhausted::retry_after`]), и назначить
            // эту паузу мигнувшей сети значило бы выключить человеку связь на час из-за секунды.
            Err(e) => {
                return Err(e).context(
                    "связь с издателем оборвалась на первой же выдаче — ни одного токена не \
                     получено (сеть?)",
                )
            }
        };
...
/// Ошибка «издатель недоступен по сети» — отдельным типом, потому что её последствия совсем не те,
/// что у отказа издателя. Отказ означает «эта ссылка больше не работает» (человеку нужен админ);
/// недоступность означает «сейчас нет связи» (человеку нужна сеть и повтор). Пока разница жила
/// только в тексте `anyhow`, интерфейс показывал первое вместо второго — и активация ссылки без
/// сети выглядела как сожжённая ссылка (см. `citadel_client::enroll`).
#[derive(Debug, Clone)]
pub struct IssuerUnreachable {
    pub addr: String,
}

impl std::fmt::Display for IssuerUnreachable {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "издатель {} недоступен (нет сети или порт закрыт)", self.addr)
    }
}
impl std::error::Error for IssuerUnreachable {}

/// Ошибка «квота выдачи на эпоху выбрана» — тоже отдельным типом, и вот почему её МОЖНО назвать
/// точно, не спрашивая издателя.
///
/// Издатель принципиально не сообщает причину отказа (различимые причины были бы оракулом по
/// политике узла — см. комментарий у [`Gate`]). Но абоненту причина и не нужна из ответа: он
/// смотрит, ГДЕ оборвалась выдача. Кадр эпохи приходит ПОСЛЕ гейта и ДО первой слепой подписи,
/// поэтому «эпоха получена, а токенов ноль» может означать ровно одно — цикл выдачи у издателя
/// прервался на квоте (`take_quota`), потому что других выходов из него нет. Отзыв и требование
/// активации приезжают раньше и отдельными кадрами, недоступность сети — ещё раньше.
///
/// Одно уточнение делает вывод честным: считаем так только при ЧИСТОМ EOF. Издатель, упёршийся в
/// квоту, дочитывает наш ослеплённый элемент и выходит из цикла — соединение закрывается штатно;
/// оборванная сеть даёт сброс или таймаут. Без этого различения секундный провал связи назначал бы
/// абоненту паузу до конца эпохи.
///
/// Отсюда и `epoch`/`epoch_secs`: они уже приехали в том же кадре, а значит клиент способен сам
/// сказать, когда идти снова, вместо того чтобы долбить издателя каждые несколько секунд до конца
/// эпохи (именно этим и выглядела жалоба «туннель больше не поднимается»).
#[derive(Debug, Clone, Copy)]
pub struct QuotaExhausted {
    /// Эпоха ПО ЧАСАМ ИЗДАТЕЛЯ, в которой квота выбрана.
    pub epoch: u64,
    /// Длина эпохи (сек) — из неё считается момент, когда квота сбросится.
    pub epoch_secs: u64,
}

impl QuotaExhausted {
    /// Сколько ждать до следующей эпохи издателя. Считается по ЕГО номеру эпохи и нашим часам,
    /// поэтому расхождение часов (а оно бывает в часы, см. `pouch_lifetime`) не должно давать ни
    /// «ждать вечно», ни «идти прямо сейчас»: зажимаем в разумные границы.
    pub fn retry_after(&self) -> std::time::Duration {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let boundary = self.epoch.saturating_add(1).saturating_mul(self.epoch_secs.max(1));
        let left = boundary.saturating_sub(now);
        std::time::Duration::from_secs(left.clamp(MIN_QUOTA_WAIT, self.epoch_secs.max(MIN_QUOTA_WAIT)))
    }
}

/// Нижняя граница паузы после исчерпанной квоты. Не ноль: смысл паузы в том, чтобы перестать
/// ходить к издателю впустую, а расхождение часов легко даёт «граница эпохи уже позади».
const MIN_QUOTA_WAIT: u64 = 60;

impl std::fmt::Display for QuotaExhausted {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "квота выдачи на эпоху {} выбрана — издатель больше не чеканит токены этому абоненту \
             до её конца (примерно {} мин). Доступ не отозван: ждать или подключаться реже",
            self.epoch,
...
mod tests {
    ...
    /// A6: пауза после исчерпанной квоты считается по эпохе ИЗДАТЕЛЯ и нашим часам, а они
    /// расходятся (в поле встречалось и три эпохи разницы). Ни «идти прямо сейчас», ни «ждать
    /// сутки» из этого получиться не должно: первое возвращает шторм заходов, который правка и
    /// убирает, второе оставляет человека без связи дольше, чем нужно.
    #[test]
    fn quota_retry_is_clamped_to_sane_window() {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        let secs = 3600;
        // Часы издателя убежали вперёд на три эпохи → граница «уже позади» → нижняя граница.
        let behind = QuotaExhausted { epoch: now / secs - 3, epoch_secs: secs };
        assert_eq!(behind.retry_after().as_secs(), MIN_QUOTA_WAIT);
        // Часы издателя отстали → «ждать три часа» тоже не годится: не дольше одной эпохи.
        let ahead = QuotaExhausted { epoch: now / secs + 3, epoch_secs: secs };
        assert!(ahead.retry_after().as_secs() <= secs);
        // Нормальный случай: ждём остаток текущей эпохи.
        let normal = QuotaExhausted { epoch: now / secs, epoch_secs: secs };
        let left = normal.retry_after().as_secs();
        assert!(left <= secs && left >= MIN_QUOTA_WAIT, "остаток эпохи, а не что попало: {left}");
    }
