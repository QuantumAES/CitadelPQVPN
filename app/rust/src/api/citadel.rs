...
#[frb(sync)]
pub fn link_is_wrapped(text: String) -> bool {
    citadel_client::masterlink::looks_wrapped(&text)
}
...
pub fn link_unwrap(block: String, password: String) -> Result<String> {
    citadel_client::masterlink::unwrap(&block, &password)
}

/// Минимальная длина пароля конверта — из ядра, чтобы поле ввода не проверяло «своё» число.
#[frb(sync)]
pub fn link_wrap_min_password() -> u32 {
    citadel_client::masterlink::MIN_PASSWORD_LEN as u32
}
