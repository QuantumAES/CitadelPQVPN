...
class AppState extends ChangeNotifier {
  ...
  /// Примета «активация не состоялась из-за СЕТИ» в тексте ошибки ядра
  /// (`citadel_client::enroll::OFFLINE_MARK`). Держим строку здесь одной константой: она —
  /// договор между ядром и интерфейсом, и менять её можно только в двух местах сразу.
  static const _offlineMark = 'нет связи с издателем';
  ...
  Future<bool> _activate(String id) async {
    try {
      ...
      // Два РАЗНЫХ отказа, и раньше они показывались одинаково — как «запросите новую ссылку».
      //   * издатель недоступен: ссылку никто не трогал, до сервера мы не дошли. Человеку нужна
      //     сеть и повтор, а не поход к администратору. Именно на это и жаловались: первое
      //     подключение без сети объявляло исправную ссылку сожжённой, а следом она спокойно
      //     активировалась;
      //   * всё остальное (просрочка, чужое устройство, отказ реестра) — ссылка действительно
      //     больше не годится.
      final offline = '$e'.contains(_offlineMark);
      _setError(
        offline ? 'err_activation_offline' : 'err_activation_failed',
        offline ? 'err_activation_offline_hint' : 'err_activation_failed_hint',
        '$e',
      );
      notifyListeners();
      return false;
