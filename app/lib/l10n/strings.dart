...
/// Пометка «локализация ещё обкатывается»: переводы не вычитаны носителями, формулировки в
/// интерфейсе VPN-клиента менять придётся. Помечены ВСЕ языки, включая русский: обкатывается сам
/// механизм локализации (строки ходят через карты и подставляются в рантайме), а не качество
/// перевода отдельного языка. Убрать = стереть эту константу и её применение в [langLabel].
const String kLangBetaSuffix = ' (beta)';

/// Название языка для интерфейса (список выбора и подпись текущего в настройках) — с пометкой
/// [kLangBetaSuffix]. Одна дверь на оба места: иначе пометка неминуемо разъехалась бы между ними.
/// Неизвестный код показываем как есть (сам код) — заметно, но экран не ломает.
String langLabel(String code) => '${kLangNames[code] ?? code}$kLangBetaSuffix';
