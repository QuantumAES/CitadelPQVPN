...
| Ключ эпохи | издатель пишет его в общий том | exit тянет по сети (сайдкар `citadel-pubsync`) |
...
* на машине A: `--issuer-port` открыт для клиентов; `--admin-port` — **только** для адреса машины B
  (`ufw allow from <IP_EXIT> to any port <admin-port> proto tcp`);
* на машине B: `--udp-port` и `--tcp-port` для клиентов; входящих от издателя не требуется —
  `citadel-pubsync` ходит исходящим соединением сам.
