...
**Admin-режим:**
- Форма SSH-подключения (host, user, пароль или ключ из keychain).
- Кнопка «Развернуть сервер» → прогресс деплоя (§8).
- «Новый клиент» → форма (имя, срок действия) → выдаёт `citadel://`-ссылку + QR + `.citadelconf`.
- Список выданных клиентов со статусом, кнопка «Отозвать» (§10).
...
endpoints (+TCP/443 fallback), **cert pin** (SHA-256, 32 B), **ML-DSA-65 pub** (1952 B), **obfs PSK** (32 B), `kx_suite`, **issuer endpoint + issuer.pub** (RSA-2048 ≈ 270 B), **Ed25519 client-seed** (Слой-1, 32 B, §10), routes/DNS/SNI.
...
> **Решение (рекомендуется): QR несёт обязательства, не ключи.** В `citadel://` кладём endpoints + `pin(32)` + `H(mldsa_pub)(32)` + `H(issuer_pub)(32)` + `obfs_psk(32)` + `ed25519_seed(32)` + мета ⇒ **~300 B** → влезает даже на высоком уровне коррекции QR. Полные публичные ключи клиент дотягивает при первом коннекте и **сверяет с хэшами из QR**.
...
- Issuer: **client-registry** + Ed25519-аутентификация перед blind-signing; `revoke` = флип статуса.
- Клиент: `TokenAgent` — фоновый рефреш, хранение пачки в `SecretStore`.
