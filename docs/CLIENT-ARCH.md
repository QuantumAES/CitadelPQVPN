...
подменяется). Меню профиля: «Переименовать» и «Переместить выше/ниже»; порядок списка — это порядок
записей в vault, поэтому он переживает перезапуск и переносится вместе с хранилищем. Всё это в
ядре (`Vault::add|rename|move_profile`), так что консольный клиент получает то же поведение.
